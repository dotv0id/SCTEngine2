...
SCTKeyboardWin32::SCTKeyboardWin32(IDirectInput8 *inputDevice)
...
SCTReturn SCTKeyboardWin32::Initialize()
{
	HRESULT hr;
	...
	if(hr != S_OK)
	{
		Core::SCTLogManager::getSingleton().PrintDebug("Failed to create keyboard input device.");
		return FAIL;
	}
	...
	hr = mpDXKeyboard->SetCooperativeLevel(Core::SCTWindows::getSingleton().GetRenderWindow(), DISCL_FOREGROUND | DISCL_EXCLUSIVE);
	if(hr != S_OK)
	{
		return FAIL;
	}
...
SCTReturn SCTKeyboardWin32::Update()
{
...
SCTReturn SCTKeyboardWin32::Shutdown()
{
...
SCTReturn SCTKeyboardWin32::UpdateKeyboardDevice()
{
	HRESULT hr;
	...
	if(hr != S_OK)
	{
		...
		{
			return FAILED_TO_ACQUIRE_KEYBOARD_DEVICE;
		}
